...
consolePanel.info('Проблема ссылочного равенства: map() всегда создаёт новый массив')
...
function checkEquality(): void {
  ...
  document.getElementById('call-count')!.textContent = String(callCount)
  ...
  if (prevResult !== null) {
    ...
    if (isEqual) {
      consolePanel.success(`prevResult === newResult → true (та же ссылка)`)
    } else {
      consolePanel.error(`prevResult === newResult → false (НОВАЯ ссылка!)`)
    }
  } else {
